...
<div class="<?php echo esc_attr( implode( ' ', apply_filters( "pirateform_wrap_classes_{$name}", $wrap_classes, $name, $args['type'] ) ) ); ?>">
	...
	<input type="tel" class="<?php echo esc_attr( apply_filters( "pirateform_field_classes_{$name}", 'form-control', $name, $args['type'] ) ); ?>" <?php echo $this->get_common( $args, array( 'value' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> >
</div>
